<?php
/**
 * The admin-specific functionality of the plugin.
 *
 * @link       https://github.com/misfist/site-functionality
 * @since      1.0.0
 *
 * @package    site-functionality
 */

namespace Site_Functionality\App\Admin;

use Site_Functionality\Common\Abstracts\Base;
use Site_Functionality\Common\WP_Includes\I18n;

/**
 * The admin-specific functionality of the plugin.
 */
// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

class Admin_Settings extends Base {

	/**
	 * Option name
	 *
	 * @var string
	 */
	public static $cta_pattern_option = 'cta_pattern';

	/**
	 * Option page slug
	 *
	 * @var string
	 */
	public static $page_slug = 'cta-settings';

	/**
	 * Constructor.
	 *
	 * @since 1.0.0
	 */
	public function __construct( $settings ) {
		parent::__construct( $settings );
		$this->init();
	}

	/**
	 * Init
	 *
	 * @return void
	 */
	public function init(): void {
		\add_action( 'acf/init', array( $this, 'acf_settings' ) );
		\add_action( 'acf/include_fields', array( $this, 'register_fields' ) );
		\add_action( 'acf/init', array( $this, 'register_options_page' ) );
	}

	/**
	 * Add fields
	 *
	 * @return void
	 */
	public function register_fields(): void {
		if ( ! function_exists( '\acf_add_local_field_group' ) ) {
			return;
		}

		\acf_add_local_field_group(
			array(
				'key'                   => 'group_ad_slot_settings',
				'title'                 => __( 'CTA Slot Settings', 'site-functionality' ),
				'fields'                => array(
					array(
						'key'                  => 'field_ad_pattern',
						'label'                => __( 'Inline CTA', 'site-functionality' ),
						'name'                 => self::$cta_pattern_option,
						'aria-label'           => '',
						'type'                 => 'post_object',
						'instructions'         => __( 'This ad will be configured for all ad slots.', 'site-functionality' ),
						'required'             => 0,
						'conditional_logic'    => 0,
						'wrapper'              => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'            => array(
							'wp_block',
						),
						'post_status'          => '',
						'taxonomy'             => '',
						'return_format'        => 'object',
						'multiple'             => 0,
						'allow_null'           => 0,
						'allow_in_bindings'    => 1,
						'bidirectional'        => 0,
						'ui'                   => 1,
						'bidirectional_target' => array(),
					),
					array(
						'key'               => 'field_cta_position',
						'label'             => __( 'Position', 'site-functionality' ),
						'name'              => 'cta_position',
						'aria-label'        => '',
						'type'              => 'range',
						'instructions'      => __( 'Number of paragraphs from top to display inline CTA.', 'site-functionality' ),
						'required'          => 0,
						'conditional_logic' => 0,
						'wrapper'           => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'default_value'     => '',
						'min'               => '',
						'max'               => '',
						'allow_in_bindings' => 0,
						'step'              => '',
						'prepend'           => '',
						'append'            => '',
					),
					array(
						'key'                  => 'field_cta_pattern_before_content',
						'label'                => __( 'Before Content CTA', 'site-functionality' ),
						'name'                 => self::$cta_pattern_option . '_before_content',
						'aria-label'           => '',
						'type'                 => 'post_object',
						'instructions'         => __( 'This ad will be configured for all ad slots.', 'site-functionality' ),
						'required'             => 0,
						'conditional_logic'    => 0,
						'wrapper'              => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'            => array(
							'wp_block',
						),
						'post_status'          => '',
						'taxonomy'             => '',
						'return_format'        => 'object',
						'multiple'             => 0,
						'allow_null'           => 0,
						'allow_in_bindings'    => 1,
						'bidirectional'        => 0,
						'ui'                   => 1,
						'bidirectional_target' => array(),
					),
					array(
						'key'                  => 'field_cta_pattern_after_content',
						'label'                => __( 'After Content CTA', 'site-functionality' ),
						'name'                 => self::$cta_pattern_option . '_after_content',
						'aria-label'           => '',
						'type'                 => 'post_object',
						'instructions'         => __( 'This ad will be configured for all ad slots.', 'site-functionality' ),
						'required'             => 0,
						'conditional_logic'    => 0,
						'wrapper'              => array(
							'width' => '',
							'class' => '',
							'id'    => '',
						),
						'post_type'            => array(
							'wp_block',
						),
						'post_status'          => '',
						'taxonomy'             => '',
						'return_format'        => 'object',
						'multiple'             => 0,
						'allow_null'           => 0,
						'allow_in_bindings'    => 1,
						'bidirectional'        => 0,
						'ui'                   => 1,
						'bidirectional_target' => array(),
					),
				),
				'location'              => array(
					array(
						array(
							'param'    => 'options_page',
							'operator' => '==',
							'value'    => self::$page_slug,
						),
					),
				),
				'menu_order'            => 0,
				'position'              => 'normal',
				'style'                 => 'seamless',
				'label_placement'       => 'top',
				'instruction_placement' => 'label',
				'hide_on_screen'        => '',
				'active'                => true,
				'description'           => '',
				'show_in_rest'          => 1,
				'display_title'         => '',
			)
		);
	}

	/**
	 * Add Page
	 *
	 * @return void
	 */
	public function register_options_page(): void {
		if ( ! function_exists( '\acf_add_options_page' ) ) {
			return;
		}

		\acf_add_options_page(
			array(
				'page_title'      => esc_html__( 'CTA Settings', 'site-functionality' ),
				'menu_slug'       => self::$page_slug,
				'parent_slug'     => 'options-general.php',
				'menu_title'      => esc_html__( 'CTA Settings', 'site-functionality' ),
				'position'        => 7,
				'redirect'        => false,
				'updated_message' => esc_attr__( 'Settings Updated', 'site-functionality' ),
				'capability'      => 'manage_options',
			)
		);
	}

	/**
	 * Add ACF settings
	 *
	 * @link https://www.advancedcustomfields.com/resources/acf-settings/
	 *
	 * @return void
	 */
	public function acf_settings(): void {
		if ( ! function_exists( '\acf_update_setting' ) ) {
			return;
		}

		\acf_update_setting( 'l10n_textdomain', 'site-functionality' );
	}
}
